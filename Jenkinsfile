...
node {
    ...
    stage ('Gradle build') {
        ...
        script {
            env.SPRING_APPLICATION_JSON = '{"spring":{"datasource":{"url":"jdbc:postgresql://postgresql:5432/bb_dev?currentSchema=payment"}}}'
        }
        try {
    ...
    stage('SonarQube analysis') {
        ...
        withSonarQubeEnv('sonarqube') {
            ...
            sh "./gradlew --info sonarqube -Dsonar.projectName=payment-service -Dsonar.projectVersion=${ver} -Dsonar.branch=${BRANCH_NAME}"
        }
    }
